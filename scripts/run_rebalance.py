...
Use ``--alpha <strategy>`` to incorporate strategy signals as
expected-return inputs to the portfolio optimiser.

Usage:
    python -m scripts.run_rebalance                          # dry run, paper
    python -m scripts.run_rebalance --execute                 # paper, executes
    python -m scripts.run_rebalance --alpha momentum           # signals-aware
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Strategy name to use as alpha signal (e.g. 'momentum')",
    )
    ...
    # ---------------------------------------------------------------
    # 2. Optimise portfolio (optionally with alpha signals)
    # ---------------------------------------------------------------
    optimiser = PortfolioOptimiser(config=config)
    method = args.method or config.get("optimisation", {}).get("default_method")

    expected_returns: pd.Series | None = None
    if args.alpha:
        signals_path = (
            Path("data") / "processed" / "signals" / f"{args.alpha}_latest.json"
        )
        if signals_path.exists():
            raw_signals = json.loads(signals_path.read_text(encoding="utf-8"))
            # Scale signals to small expected returns
            expected_returns = pd.Series(
                {t: s * 0.01 for t, s in raw_signals.items() if t in returns.columns},
                name="expected_returns",
            )
            logger.info(
                "Using alpha signals from '%s' (%d tickers)",
                args.alpha, len(expected_returns),
            )
        else:
            logger.warning(
                "Signal file not found: %s -- running without alpha", signals_path,
            )

    target_weights = optimiser.optimise(
        returns, method=method, expected_returns=expected_returns,
    )
