...
def load_config(path: Path | str | None = None) -> dict[str, Any]:
    ...
    _resolve_dynamic_dates(config)
    return config


def _resolve_dynamic_dates(config: dict[str, Any]) -> None:
    """Resolve dynamic date placeholders in the data section.

    Rules:
    - ``"today"`` is replaced with the current date (YYYY-MM-DD).
    - A date string more than 30 days in the past is replaced with today.
    - ``None`` is left as-is (some fetchers treat None as "up to now").
    """
    ...
    if isinstance(end_date, str) and end_date.strip().lower() == "today":
        data["end_date"] = today_str
        return

    # If it's a date string, check staleness (>30 days old -> replace)
    try:
        parsed = datetime.date.fromisoformat(str(end_date))
        if (datetime.date.today() - parsed).days > 30:
            data["end_date"] = today_str
